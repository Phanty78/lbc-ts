import {
  AdType,
  type AdType as AdTypeValue,
  Category,
  type Category as CategoryValue,
  type Department as DepartmentValue,
  type OwnerType as OwnerTypeValue,
  type Region as RegionValue,
  Sort,
  type Sort as SortValue,
} from "./enums.js";
...
const nestedFilters = (current: Filters, key: string): Filters => {
  const existing = current[key];
  if (existing && typeof existing === "object" && !Array.isArray(existing)) return existing as Filters;
...
export function buildSearchPayloadFromUrl(url: string, limit = 35, limitAlu = 3, page = 1): SearchPayload {
  ...
  try {
    ...
    throw new InvalidValueError("The URL must be a valid Leboncoin search URL.");
  }
  const payload = basePayload(limit, limitAlu, page);
  for (const [key, value] of parsed.searchParams) {
    const current = filters(payload);
    if (key === "text") {
      current.keywords = { text: value };
      continue;
    }
    if (key === "category") {
    ...
    if (max !== undefined && rest.length === 0) {
      ...
      if (Object.keys(range).length) {
        const ranges = nestedFilters(current, "ranges");
        ranges[key] = range;
      }
      continue;
    }
    const enums = nestedFilters(current, "enums");
...
function addLocations(payload: SearchPayload, locations: readonly Location[]): void {
  ...
  for (const location of locations) {
    if (location instanceof City) {
      items.push({
        locationType: "city",
        area: { lat: location.lat, lng: location.lng, radius: location.radius },
        city: location.city,
        label: location.city ? `${location.city} (toute la ville)` : undefined,
      });
      continue;
    }
    if (Array.isArray(location) && location.length === 2) {
      items.push({ locationType: "region", region_id: location[0] });
      continue;
    }
    if (Array.isArray(location) && location.length === 4) {
      items.push({ locationType: "department", region_id: location[0], department_id: location[2] });
      continue;
    }
...
function addUrlLocations(payload: SearchPayload, value: string): void {
  ...
  for (const location of value.split(",")) {
    ...
    if (prefix === "d" && id) {
      items.push({ locationType: "department", department_id: id });
      continue;
    }
    if (prefix === "r" && id) {
      items.push({ locationType: "region", region_id: id });
      continue;
    }
    if (prefix === "p" && id && area) {
      items.push({ locationType: "place", place: id, label: id, area: buildArea(area) });
      continue;
    }
...
function addAdvancedFilter(payload: SearchPayload, key: string, value: unknown): void {
  ...
  if (value.every((item): item is string => typeof item === "string")) {
    nestedFilters(current, "enums")[key] = value;
    return;
  }
...
function toInteger(value: string): number {
  ...
  if (!Number.isFinite(parsed)) throw new InvalidValueError(`Invalid number: ${value}`);
  return parsed;
}
