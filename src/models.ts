...
export class Ad {
  ...
  constructor(raw: Raw, client: ClientLike) {
    ...
    this.location = {
      countryId: string(location.country_id),
      regionId: string(location.region_id),
      regionName: string(location.region_name),
      departmentId: string(location.department_id),
      departmentName: string(location.department_name),
      cityLabel: string(location.city_label),
      ...
      isShape: boolean(location.is_shape),
    };
...
export class User {
  ...
  constructor(userData: Raw, proData?: Raw) {
    ...
    const overallScore = number(feedback.overall_score);
    ...
    this.feedback = {
      overallScore,
      cleanness: number(scores.CLEANNESS),
      communication: number(scores.COMMUNICATION),
      conformity: number(scores.CONFORMITY),
      package: number(scores.PACKAGE),
      product: number(scores.PRODUCT),
      recommendation: number(scores.RECOMMENDATION),
      respect: number(scores.RESPECT),
      transaction: number(scores.TRANSACTION),
      userAttention: number(scores.USER_ATTENTION),
      receivedCount: number(feedback.received_count),
      score: overallScore === undefined ? undefined : overallScore * 5,
    };
    this.profilePicture = string(record(userData.profile_picture).extra_large_url);
    this.reply = {
      inMinutes: number(reply.in_minutes),
      text: string(reply.text),
      rateText: string(reply.rate_text),
      rate: number(reply.rate),
      replyTimeText: string(reply.reply_time_text),
    };
    this.presence = {
      status: string(presence.status),
      presenceText: string(presence.presence_text),
      lastActivity: string(presence.last_activity),
      enabled: boolean(presence.enabled),
    };
    this.badges = (Array.isArray(userData.badges) ? userData.badges : []).map((badge) => ({
      type: string(record(badge).type),
      name: string(record(badge).name),
    }));
...
function buildPro(raw: Raw): Pro {
  ...
  return {
    onlineStoreId: number(raw.online_store_id),
    onlineStoreName: string(raw.online_store_name),
    activitySectorId: number(owner.activitySectorID),
    activitySector: string(owner.activitySector),
    categoryId: number(owner.categoryId),
    siren: string(owner.siren),
    siret: string(owner.siret),
    storeId: number(owner.storeId),
    activeSince: string(owner.activeSince),
    location: {
      ...
      lng: number(location.lng),
      zipcode: string(location.zipcode),
      geoSource: string(location.geo_source),
      geoProvider: string(location.geo_provider),
      region: string(location.region),
      regionLabel: string(location.region_label),
      department: string(location.department),
      departmentLabel: string(location.dpt_label),
      country: string(location.country),
    },
    logo: string(record(brand.logo).large),
    cover: string(record(brand.cover).large),
    slogan: string(brand.slogan),
    description: string(information.description),
    openingHours: string(information.opening_hours),
    websiteUrl: string(information.website_url),
    rating: {
      ratingValue: number(rating.rating_value),
      userRatingsTotal: number(rating.user_ratings_total),
      source: string(rating.source),
      sourceDisplay: string(rating.source_display),
      retrievalTime: string(rating.retrieval_time),
      url: string(rating.url),
      reviews: (Array.isArray(rating.reviews) ? rating.reviews : []).map((review) => ({
        authorName: string(record(review).author_name),
        ratingValue: number(record(review).rating_value),
        text: string(record(review).text),
        reviewTime: string(record(review).review_time),
      })),
    },
  };
}
